  ...
  return (
    <>
      {ratingValues.map((rank) => (
        <Fragment key={rank}>
          <input className="form__rating-input visually-hidden" name="rating" value={rank} id={`${rank}-star${(rank > 1) ? 's' : ''}`} type="radio" checked={rank === rating} onChange={onRatingChange} />
