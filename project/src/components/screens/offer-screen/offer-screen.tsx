...
function OfferScreen({authStatus, currentOffer, isCurrentOfferLoaded, nearbyOffers, isNearbyLoaded, fetchCurrentOffer, fetchNearbyOffers}: PropsFromRedux): JSX.Element {
  ...
  }, [fetchCurrentOffer, fetchNearbyOffers, paramsProps.id]);
  ...
  const closestOffers: OfferType[] = offers.filter((offer) => offer.id !== +paramsProps.id).slice(0, 3);
